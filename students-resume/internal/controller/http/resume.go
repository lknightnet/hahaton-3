...
func NewResumeRoutes(studentUC service.Resume, r *mux.Router) {
	...
	r.HandleFunc("/resume/get/{id:[0-9]+}", studentCtrl.getResume).Methods(http.MethodGet)
	r.HandleFunc("/resume/create", studentCtrl.createResume).Methods(http.MethodPost)
}
